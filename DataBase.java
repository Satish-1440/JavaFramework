...
class CRUDOperations
{
    ...
    public static void showRecordDetails()
    {
        try
        {
            rs = stmt.executeQuery("select * from " + tableName + " where " + columnNames[columnNames.length - 1]+ " = '" + activeStatus + "'");
            header = "";
            printColumnNames();
            printValues();
        }
    ...
    public static void searchRecord()
    {
        try
        {
            ...
            printColumnNames();
            query = "select * from "+ tableName +" where " + columnNames[0] + " = '" + uniqueValue + "' and " + columnNames[columnNames.length - 1] + " = '" + activeStatus + "'";
    ...
    public static void updateRecordDetails()
    {
        ...
        try
        {
            if(choice > 0 && choice < columnCount - 1 )
            {
                ...
                scanner.next();
                query = "update " + tableName + " set " + columnNames[choice] + " = '"+ updated_column_value + "' where " + columnNames[0] + " = '" + uniqueValue + "' and " + columnNames[columnNames.length - 1] + " = '" + activeStatus + "'";
